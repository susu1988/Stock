...
import com.susu.dao.AgvDao;

public class AdminActionAgv  extends BaseAction {
	 ...
	 * @return String
	 */
	public String listAgvs(){
		try {
			if (paramsAgv==null) {
				paramsAgv = new Agv();
			}
			//设置分页信息
			setPagination(paramsAgv);
			//总的条数
			int[] sum={0};
			//查询车辆列表
			List<Agv> Agvs = adminManager.listAgvs(paramsAgv,sum); 
			
			Param.setAttribute("Agvs", Agvs);
			setTotalCount(sum[0]);
	...
	public String addAgv(){
		try {
			...
			if (Agv!=null) {
				tip="失败，该车牌号已经存在！";
				Param.setAttribute("Agv", paramsAgv);
				return "AgvEdit";
			}
			 //添加车辆
			adminManager.addAgv(paramsAgv);
			
			setSuccessTip("添加成功", "Admin_listAgvs.action");
		} catch (Exception e) {
			setErrorTip("添加车辆异常", "Admin_listAgvs.action");
		}
	 ...
	 * @return String
	 */
	public String editAgv(){
		try {
			 //得到车辆
			Agv Agv = adminManager.queryAgv(paramsAgv);
			Param.setAttribute("Agv", Agv);
			
		} catch (Exception e) {
			setErrorTip("查询车辆异常", "Admin_listAgvs.action");
			return "infoTip";
		}
	 ...
	 * @return String
	 */
	public String saveAgv(){
		try {
			 //保存编辑车辆
			adminManager.updateAgv(paramsAgv);
			
			setSuccessTip("编辑成功", "Admin_listAgvs.action");
		} catch (Exception e) {
			tip="编辑失败";
			Param.setAttribute("Agv", paramsAgv);
			return "AgvEdit";
	 ...
	 * @return String
	 */
	public String delAgvs(){
		try {
			 //删除车辆
			adminManager.delAgvs(paramsAgv);
			
			setSuccessTip("删除车辆成功", "Admin_listAgvs.action");
		} catch (Exception e) {
			setErrorTip("删除车辆异常", "Admin_listAgvs.action");
		}
	...
	public String addOrders(){
		try {
			...
			setErrorTip("添加订单异常", "Admin_listOrderss.action");
		}
	...
	public String confirmOrdersShow(){
		try {
			...
			Agv.setAgvFlag(1);
			Agv.setStart(-1);
			List<Agv> Agvs = adminManager.listAgvs(Agv, null);
			if (Agvs==null) {
	...
	public Agv getParamsAgv() {
		return paramsAgv;
	}

	public void setParamsAgv(Agv paramsAgv) {
		this.paramsAgv = paramsAgv;
	}
